...
UI Lab（`pnpm dev:ui-lab`）の「実装相談」サンプル、または Electron の Agent 回答で確認:

- [ ] 単一行・複数行・言語指定あり（` ```ts `）のフェンスブロックに `Copy` ボタンがある
- [ ] クリック後ペーストでブロック本文と一致する
- [ ] インライン `` `code` `` にはボタンが付かない
- [ ] ストリーミング完了前後でコピー内容が期待通り
- [ ] リンク付き Markdown とコードブロックが同居するメッセージ
- [ ] 履歴セッションを開き直した後もコピー可能
...
| ---------------------------------- | ---------------------------------------------------------------- | --------------------------------------------- |
| チャット見た目・タブ               | ○                                                                | ○                                             |
| 横長ログ・広いテーブル（UI Lab）   | ○ — `横長コンテンツ` セッションで横スクロール確認                | ○                                             |
